...
display(len(nobel)) # returns 911

# Display the number of prizes won by male and female recipients.
display(nobel['sex'].value_counts()) # returns male 836 / female 49
...
#display(nobel['usa_born_winner'])
#nobel['decade'] = np.floor()
nobel['decade'] = np.int64(np.floor(nobel['year']))
#display(nobel['decade'])
...
#PercentFormatter(xmax=100, decimals=None, symbol='%', is_latex=False)[source]
ax.yaxis.set_major_formatter(PercentFormatter()) # returns yaxis in pct ok.
...
nobel['female_winner'] = nobel.groupby('sex')=='Female'
...
nobel.nsmallest(49,'female_winner')
...
# Selecting the laureates that have received 2 or more prize
nobel.groupby('full_name').filter(lambda nobel: len(nobel)>=2)
