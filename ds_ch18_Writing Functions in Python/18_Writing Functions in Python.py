...
# 18_Writing Functions in Python
...
#6)Best practice for default arguments
...
def timer():
  ...
  """
  start = time.time()
  # Send control back to the context block (use yield)
  yield start
  end = time.time()
...
#4)A read-only open() context manager
@contextlib.contextmanager
...
'''
___________________________________________________________________________________
##  Part IV More on Decorators
___________________________________________________________________________________
'''
 
#1)Print the return type
...
#3)Preserving docstrings when decorating functions
...
#4)Measuring decorator overhead
@check_everything
